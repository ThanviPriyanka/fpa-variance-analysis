...
SEGS = ["Home & Decor", "Kitchen & Dining", "Storage", "Bags", "Lighting", "Party & Seasonal", "Stationery & Craft", "Other"]
N = len(SEGS)
...
fact = pd.read_csv("data/processed/monthly_fact_by_category.csv")
wsd = wb.active; wsd.title = "Data"
wsd["A1"] = "MONTHLY FACT TABLE  —  output of sql/build_fact_table_by_category.sql (DuckDB)"; wsd["A1"].font = TITLE
...
wsa["A5"] = "Through month (period cutoff)"; wsa["A5"].font = BOLD
wsa["B5"] = 11; wsa["B5"].font = BLUE; wsa["B5"].fill = YEL; wsa["B5"].number_format = "0"
wsa["C5"] = "Set to 11: December 2011 is truncated in the source, so compare like-for-like (Jan-Nov)."
wsa["C5"].font = Font(name=ARIAL, italic=True, color="808080")
...
wsb["A1"] = "FY2011 Revenue Variance  —  Plan vs Actual  (by product category)"; wsb["A1"].font = Font(name=ARIAL, size=16, bold=True, color=INK)
wsb["A2"] = ("Finding: revenue came in ~9% below a +10% growth plan, driven by a volume shortfall (-£1.42M). Pricing (+£0.35M) "
             "and mix (+£0.18M) partly offset it. Unlike the by-market view, product mix is now a real story: customers shifted "
             "toward Storage and Home & Decor and away from Kitchen & Dining and the long tail. Like-for-like Jan–Nov.")
...
notes = ["", "DATA SOURCE",
 ...
 "• Pipeline: sql/build_fact_table_by_category.sql (DuckDB).", "",
 ...
 "SEGMENTATION  (v2 'by product category' cut)",
 "• Segment = product category derived from item descriptions via keyword rules in the SQL.",
 "• Eight categories: Home & Decor, Kitchen & Dining, Storage, Bags, Lighting, Party & Seasonal, Stationery & Craft, Other.",
 "• Non-product lines (postage, carriage, manual adjustments, gift vouchers, ~GBP 0.84M) are excluded.",
 "• 'Other' is ~23% of revenue (genuine long-tail miscellany); categories are balanced, so mix is a real driver here.", "",
 "COMPARISON PERIOD",
...
wb.active = 0
wb.save("model/FPA_Variance_by_category.xlsx")
print("saved")
